...
[`AGENTS.md`](https://ehrlinger.github.io/hvtiRtables/AGENTS.md),
...
(`/codemap hvtiRtables`). If the codemap looks stale, say so and offer
...
it, then orient from the repo itself — `NAMESPACE`, `R/`,
`CONTRIBUTING.md` — before editing.

## Editing one renderer

`AGENTS.md` requires that a change to one renderer be made to the other.
In practice: after touching `hv-man-table.R` or `hv-man-table-jtcvs.R`,
run
...
before anything else. It is fast, and it is the test that fails when the
two error contracts drift by a word — which is easy to do when editing
only the file you were asked about.
