...
| `PRINTFN=` | `footnotes = hv_man_footnotes()` | **CORR only.** The house block is CORR-shaped and cannot be passed to `hv_man_table_save_jtcvs()`, which needs `list(row =, col =, text =)` entries; build those, or use `hv_test_footnotes_jtcvs()`. On the standard `hv_tbl_summary()` -> `hv_man_table()` path, the default dagger follows `continuous_stat` and `percentiles` automatically. |
