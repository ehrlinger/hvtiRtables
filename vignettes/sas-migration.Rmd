...
| `PRINTFN=` | `footnotes = hv_man_footnotes()` | **CORR only.** The house block is CORR-shaped and cannot be passed to `hv_man_table_save_jtcvs()`, which needs `list(row =, col =, text =)` entries; build those, or use `hv_test_footnotes_jtcvs()`. It also hard-codes the 15th/85th percentiles, so change it if you change `percentiles` or set `continuous_stat = "mean"` |
